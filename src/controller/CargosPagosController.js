    ...
    methods: {
        ...
        async seleccionarItemCurso(itemCurso, cat_cargo) {
            ...
            if (itemCurso && cat_cargo == CONSTANTES.ID_CARGO_COLEGIATURA) {
                ...
                if (this.cargo.co_curso && this.cargo.co_curso.cat_esquema_pago == 1) {
                ...
                if (this.cargo.co_curso && this.cargo.co_curso.cat_esquema_pago == 2) {

                    this.listaMesesAdeuda = await this.getAsync(`${URL.PERIODOS_CURSO}/meses/pendientes/${this.cargo.co_curso.uid_curso}/${this.alumno.id}`);
                }
        ...
        async guardarCargo() {
            ...
            if (this.cargo.cat_cargo.id == 1) { //colegiatura
                ...
                if (this.cargo.co_curso_semanas == -1 && this.cargo.co_curso.cat_esquema_pago == 1) {
                    this.$notificacion.error('Seleccione la semana correspondiente a la colegiatura.', '');
                    return;
                }

                if (this.cargo.mes == null && this.cargo.co_curso.cat_esquema_pago == 2) {
            ...
            const result = await this.postAsync(URL.CARGO_REGISTRAR, {
                fecha: fecha,
                cat_esquema_pago: this.cargo.co_curso.cat_esquema_pago,
