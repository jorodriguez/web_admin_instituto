...
function castNumDayToSpanish(num_day){
    if(numeroValido(num_day) && (num_day >= 0 && num_day <= 6)){
...
function castNumMonthToSpanish(num_month){
    ...
    if(numeroValido(num_month) && (num_month >= 1 && num_month <= 12) ){
        let re =  MESES.find(e=>e.numero == num_month);;
        console.log("retorno "+re);
        return re;
    }
...
export { castNumDayToSpanish, castNumMonthToSpanish };